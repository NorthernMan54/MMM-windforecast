...
Module.register("MMM-windforecast", {
  ...
  defaults: {
    ...
    showWindAmount: false,
    updateInterval: .25 * 60 * 1000, // every 15 seconds
    animationSpeed: 1000,
    ...
    colored: false,
    scale: false,
    daily: true,      // toggle for daily vs hourly
  ...
  getDom: function() {
    ...
    for (var f in this.forecast) {
      ...
      if (this.config.showWindAmount) {
        var windCell = document.createElement("td");
        if (isNaN(forecast.wind)) {
          windCell.innerHTML = "";
        } else {
          if (config.units !== "imperial") {
            windCell.innerHTML = (parseFloat(forecast.wind) * 3.6).toFixed(1).replace(".", this.config.decimalSymbol) + " KPh";
          } else {
            windCell.innerHTML = (parseFloat(forecast.wind) * 2.237).toFixed(2).replace(".", this.config.decimalSymbol) + " Mph";
          }
        }
        windCell.className = "align-right bright wind";
  ...
  processRain: function(forecast, allForecasts) {
    ...
    if (daysForecasts.length == 0) {
      // Log.log("Rain-3", forecast.rain);
      return undefined;
    }

    // Summarize all the rain from the matching days
    // Log.log("Rain-4", forecast.rain);
